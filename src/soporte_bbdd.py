...
def insertar_datos(query, contraseña, nombre_bbdd, datos):
    """
    Inserta datos en una base de datos utilizando una consulta y tupla como valores.
    ...
    - nombre_bbdd (str): Nombre de la base de datos a la que se conectará.
    - datos (tupla): Tupla con los datos a insertar.

    Returns:
    - None: No devuelve ningún valor, pero inserta los datos en la base de datos.

    This function connects to a MySQL database using the given credentials, executes the query with the provided tuple, and commits the changes to the database. In case of an error, it prints the error details.
    """
    cnx = mysql.connector.connect(
        user="root", 
        password=contraseña, 
...
def insertar_datos_multiple(query, contraseña, nombre_bbdd, lista_tuplas):
    """
